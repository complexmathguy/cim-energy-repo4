...
public class RotatingMachineCommandRestController extends BaseSpringRestController {
	...
	public void addToRotatingMachine( @RequestBody(required=true) AssignRotatingMachineToRotatingMachineCommand command ) {
		try {
	...
	public void removeFromRotatingMachine( 	@RequestBody(required=true) RemoveRotatingMachineFromRotatingMachineCommand command )
	{		
		try {
			RotatingMachineBusinessDelegate.getRotatingMachineInstance().removeFromRotatingMachine( command );
		}
		catch( Exception exc ) {
