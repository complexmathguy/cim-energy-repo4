...
public class AccumulatorResetCommandRestController extends BaseSpringRestController {
	...
	public void assignAccumulatorReset( @RequestBody AssignAccumulatorResetToAccumulatorResetCommand command ) {
		try {
			AccumulatorResetBusinessDelegate.getAccumulatorResetInstance().assignAccumulatorReset( command );   
		}
        catch( Throwable exc ) {
	...
	public void unAssignAccumulatorReset( @RequestBody(required=true)  UnAssignAccumulatorResetFromAccumulatorResetCommand command ) {
		try {
			AccumulatorResetBusinessDelegate.getAccumulatorResetInstance().unAssignAccumulatorReset( command );   
		}
		catch( Exception exc ) {
