...
public class AccumulatorLimitSetCommandRestController extends BaseSpringRestController {
	...
	public void addToLimitSets( @RequestBody(required=true) AssignLimitSetsToAccumulatorLimitSetCommand command ) {
		try {
	...
	public void removeFromLimitSets( 	@RequestBody(required=true) RemoveLimitSetsFromAccumulatorLimitSetCommand command )
	{		
		try {
			AccumulatorLimitSetBusinessDelegate.getAccumulatorLimitSetInstance().removeFromLimitSets( command );
		}
		catch( Exception exc ) {
