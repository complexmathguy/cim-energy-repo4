...
public class RegulatingCondEqAggregate {  
    ...
    public void handle(UpdateRegulatingCondEqCommand command) throws Exception {
    	...
    	UpdateRegulatingCondEqEvent event = new UpdateRegulatingCondEqEvent(command.getRegulatingCondEqId(), command.getRegulatingCondEq());        
    ...
    public void handle(AssignRegulatingCondEqToRegulatingCondEqCommand command) throws Exception {
    	...
    	if ( regulatingCondEq.contains( command.getAddTo() ) )
    		throw new ProcessingException( "RegulatingCondEq already contains an entity with id " + command.getAddTo().getRegulatingCondEqId() );

    	apply(new AssignRegulatingCondEqToRegulatingCondEqEvent(command.getRegulatingCondEqId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveRegulatingCondEqFromRegulatingCondEqCommand command) throws Exception {
    	...
    	if ( !regulatingCondEq.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "RegulatingCondEq does not contain an entity with id " + command.getRemoveFrom().getRegulatingCondEqId() );

        apply(new RemoveRegulatingCondEqFromRegulatingCondEqEvent(command.getRegulatingCondEqId(), command.getRemoveFrom()));
    }
