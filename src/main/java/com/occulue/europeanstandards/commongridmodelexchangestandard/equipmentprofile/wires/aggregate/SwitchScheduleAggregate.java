...
public class SwitchScheduleAggregate {  
    ...
    public void handle(UpdateSwitchScheduleCommand command) throws Exception {
    	...
    	UpdateSwitchScheduleEvent event = new UpdateSwitchScheduleEvent(command.getSwitchScheduleId(), command.getSwitchSchedules());        
    ...
    public void handle(AssignSwitchSchedulesToSwitchScheduleCommand command) throws Exception {
    	...
    	if ( switchSchedules.contains( command.getAddTo() ) )
    		throw new ProcessingException( "SwitchSchedules already contains an entity with id " + command.getAddTo().getSwitchScheduleId() );

    	apply(new AssignSwitchSchedulesToSwitchScheduleEvent(command.getSwitchScheduleId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveSwitchSchedulesFromSwitchScheduleCommand command) throws Exception {
    	...
    	if ( !switchSchedules.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "SwitchSchedules does not contain an entity with id " + command.getRemoveFrom().getSwitchScheduleId() );

        apply(new RemoveSwitchSchedulesFromSwitchScheduleEvent(command.getSwitchScheduleId(), command.getRemoveFrom()));
    }
