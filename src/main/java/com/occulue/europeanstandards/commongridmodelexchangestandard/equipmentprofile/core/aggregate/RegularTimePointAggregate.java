...
public class RegularTimePointAggregate {  
    ...
    public void handle(UpdateRegularTimePointCommand command) throws Exception {
    	...
    	UpdateRegularTimePointEvent event = new UpdateRegularTimePointEvent(command.getRegularTimePointId(), command.getSequenceNumber(), command.getValue1(), command.getValue2(), command.getTimePoints());        
    ...
    public void handle(AssignTimePointsToRegularTimePointCommand command) throws Exception {
    	...
    	if ( timePoints.contains( command.getAddTo() ) )
    		throw new ProcessingException( "TimePoints already contains an entity with id " + command.getAddTo().getRegularTimePointId() );

    	apply(new AssignTimePointsToRegularTimePointEvent(command.getRegularTimePointId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveTimePointsFromRegularTimePointCommand command) throws Exception {
    	...
    	if ( !timePoints.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "TimePoints does not contain an entity with id " + command.getRemoveFrom().getRegularTimePointId() );

        apply(new RemoveTimePointsFromRegularTimePointEvent(command.getRegularTimePointId(), command.getRemoveFrom()));
    }
