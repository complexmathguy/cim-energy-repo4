...
public class AccumulatorValueAggregate {  
    ...
    public void handle(UpdateAccumulatorValueCommand command) throws Exception {
    	...
    	UpdateAccumulatorValueEvent event = new UpdateAccumulatorValueEvent(command.getAccumulatorValueId(), command.getValue(), command.getAccumulatorValues());        
    ...
    public void handle(AssignAccumulatorValuesToAccumulatorValueCommand command) throws Exception {
    	...
    	if ( accumulatorValues.contains( command.getAddTo() ) )
    		throw new ProcessingException( "AccumulatorValues already contains an entity with id " + command.getAddTo().getAccumulatorValueId() );

    	apply(new AssignAccumulatorValuesToAccumulatorValueEvent(command.getAccumulatorValueId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveAccumulatorValuesFromAccumulatorValueCommand command) throws Exception {
    	...
    	if ( !accumulatorValues.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "AccumulatorValues does not contain an entity with id " + command.getRemoveFrom().getAccumulatorValueId() );

        apply(new RemoveAccumulatorValuesFromAccumulatorValueEvent(command.getAccumulatorValueId(), command.getRemoveFrom()));
    }
