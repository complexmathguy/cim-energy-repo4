...
public class VsConverterAggregate {  
    ...
    public void handle(UpdateVsConverterCommand command) throws Exception {
    	...
    	UpdateVsConverterEvent event = new UpdateVsConverterEvent(command.getVsConverterId(), command.getMaxModulationIndex(), command.getMaxValveCurrent(), command.getVsConverterDCSides());        
    ...
    public void handle(AssignVsConverterDCSidesToVsConverterCommand command) throws Exception {
    	...
    	if ( vsConverterDCSides.contains( command.getAddTo() ) )
    		throw new ProcessingException( "VsConverterDCSides already contains an entity with id " + command.getAddTo().getVsConverterId() );

    	apply(new AssignVsConverterDCSidesToVsConverterEvent(command.getVsConverterId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveVsConverterDCSidesFromVsConverterCommand command) throws Exception {
    	...
    	if ( !vsConverterDCSides.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "VsConverterDCSides does not contain an entity with id " + command.getRemoveFrom().getVsConverterId() );

        apply(new RemoveVsConverterDCSidesFromVsConverterEvent(command.getVsConverterId(), command.getRemoveFrom()));
    }
