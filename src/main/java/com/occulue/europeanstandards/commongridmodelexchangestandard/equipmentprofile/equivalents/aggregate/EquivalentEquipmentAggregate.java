...
public class EquivalentEquipmentAggregate {  
    ...
    public void handle(UpdateEquivalentEquipmentCommand command) throws Exception {
    	...
    	UpdateEquivalentEquipmentEvent event = new UpdateEquivalentEquipmentEvent(command.getEquivalentEquipmentId(), command.getEquivalentEquipments());        
    ...
    @CommandHandler
    public void handle(AssignEquivalentEquipmentsToEquivalentEquipmentCommand command) throws Exception {
    	...
    	if ( equivalentEquipments.contains( command.getAddTo() ) )
    		throw new ProcessingException( "EquivalentEquipments already contains an entity with id " + command.getAddTo().getEquivalentEquipmentId() );

    	apply(new AssignEquivalentEquipmentsToEquivalentEquipmentEvent(command.getEquivalentEquipmentId(), command.getAddTo()));
    }

    @CommandHandler
    public void handle(RemoveEquivalentEquipmentsFromEquivalentEquipmentCommand command) throws Exception {
    	...
    	if ( !equivalentEquipments.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "EquivalentEquipments does not contain an entity with id " + command.getRemoveFrom().getEquivalentEquipmentId() );

        apply(new RemoveEquivalentEquipmentsFromEquivalentEquipmentEvent(command.getEquivalentEquipmentId(), command.getRemoveFrom()));
    }
