...
public class AccumulatorLimitSetAggregate {  
    ...
    public void handle(UpdateAccumulatorLimitSetCommand command) throws Exception {
    	...
    	UpdateAccumulatorLimitSetEvent event = new UpdateAccumulatorLimitSetEvent(command.getAccumulatorLimitSetId(), command.getLimitSets());        
    ...
    public void handle(AssignLimitSetsToAccumulatorLimitSetCommand command) throws Exception {
    	...
    	if ( limitSets.contains( command.getAddTo() ) )
    		throw new ProcessingException( "LimitSets already contains an entity with id " + command.getAddTo().getAccumulatorLimitSetId() );

    	apply(new AssignLimitSetsToAccumulatorLimitSetEvent(command.getAccumulatorLimitSetId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveLimitSetsFromAccumulatorLimitSetCommand command) throws Exception {
    	...
    	if ( !limitSets.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "LimitSets does not contain an entity with id " + command.getRemoveFrom().getAccumulatorLimitSetId() );

        apply(new RemoveLimitSetsFromAccumulatorLimitSetEvent(command.getAccumulatorLimitSetId(), command.getRemoveFrom()));
    }
