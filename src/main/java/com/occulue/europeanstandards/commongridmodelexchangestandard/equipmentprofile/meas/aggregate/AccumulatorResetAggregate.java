...
public class AccumulatorResetAggregate {  
    ...
    public void handle(UpdateAccumulatorResetCommand command) throws Exception {
    	...
    	UpdateAccumulatorResetEvent event = new UpdateAccumulatorResetEvent(command.getAccumulatorResetId(), command.getAccumulatorReset());        
    ...
    public void handle(AssignAccumulatorResetToAccumulatorResetCommand command) throws Exception {
    	...
    	if (  accumulatorReset != null && accumulatorReset.getAccumulatorResetId() == command.getAssignment().getAccumulatorResetId() )
    		throw new ProcessingException( "AccumulatorReset already assigned with id " + command.getAssignment().getAccumulatorResetId() );  
    		
        apply(new AssignAccumulatorResetToAccumulatorResetEvent(command.getAccumulatorResetId(), command.getAssignment()));
    }
