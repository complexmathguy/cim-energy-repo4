...
public class BusNameMarkerAggregate {  
    ...
    public void handle(UpdateBusNameMarkerCommand command) throws Exception {
    	...
    	UpdateBusNameMarkerEvent event = new UpdateBusNameMarkerEvent(command.getBusNameMarkerId(), command.getPriority(), command.getBusNameMarker());        
    ...
    public void handle(AssignBusNameMarkerToBusNameMarkerCommand command) throws Exception {
    	...
    	if ( busNameMarker.contains( command.getAddTo() ) )
    		throw new ProcessingException( "BusNameMarker already contains an entity with id " + command.getAddTo().getBusNameMarkerId() );

    	apply(new AssignBusNameMarkerToBusNameMarkerEvent(command.getBusNameMarkerId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveBusNameMarkerFromBusNameMarkerCommand command) throws Exception {
    	...
    	if ( !busNameMarker.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "BusNameMarker does not contain an entity with id " + command.getRemoveFrom().getBusNameMarkerId() );

        apply(new RemoveBusNameMarkerFromBusNameMarkerEvent(command.getBusNameMarkerId(), command.getRemoveFrom()));
    }
