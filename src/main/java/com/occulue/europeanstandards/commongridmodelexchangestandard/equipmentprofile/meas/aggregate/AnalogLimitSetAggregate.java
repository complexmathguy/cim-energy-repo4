...
public class AnalogLimitSetAggregate {  
    ...
    public void handle(UpdateAnalogLimitSetCommand command) throws Exception {
    	...
    	UpdateAnalogLimitSetEvent event = new UpdateAnalogLimitSetEvent(command.getAnalogLimitSetId(), command.getLimitSets());        
    ...
    public void handle(AssignLimitSetsToAnalogLimitSetCommand command) throws Exception {
    	...
    	if ( limitSets.contains( command.getAddTo() ) )
    		throw new ProcessingException( "LimitSets already contains an entity with id " + command.getAddTo().getAnalogLimitSetId() );

    	apply(new AssignLimitSetsToAnalogLimitSetEvent(command.getAnalogLimitSetId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveLimitSetsFromAnalogLimitSetCommand command) throws Exception {
    	...
    	if ( !limitSets.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "LimitSets does not contain an entity with id " + command.getRemoveFrom().getAnalogLimitSetId() );

        apply(new RemoveLimitSetsFromAnalogLimitSetEvent(command.getAnalogLimitSetId(), command.getRemoveFrom()));
    }
