...
public class MeasurementValueQualityAggregate {  
    ...
    public void handle(UpdateMeasurementValueQualityCommand command) throws Exception {
    	...
    	UpdateMeasurementValueQualityEvent event = new UpdateMeasurementValueQualityEvent(command.getMeasurementValueQualityId(), command.getMeasurementValueQuality());        
    ...
    @CommandHandler
    public void handle(AssignMeasurementValueQualityToMeasurementValueQualityCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignMeasurementValueQualityToMeasurementValueQualityCommand" );
    	
    	if (  measurementValueQuality != null && measurementValueQuality.getMeasurementValueQualityId() == command.getAssignment().getMeasurementValueQualityId() )
    		throw new ProcessingException( "MeasurementValueQuality already assigned with id " + command.getAssignment().getMeasurementValueQualityId() );  
    		
        apply(new AssignMeasurementValueQualityToMeasurementValueQualityEvent(command.getMeasurementValueQualityId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignMeasurementValueQualityFromMeasurementValueQualityCommand command) throws Exception {
    	LOGGER.info( "Handlign command UnAssignMeasurementValueQualityFromMeasurementValueQualityCommand" );

    	if (  measurementValueQuality == null )
    		throw new ProcessingException( "MeasurementValueQuality already has nothing assigned." );  

    	apply(new UnAssignMeasurementValueQualityFromMeasurementValueQualityEvent(command.getMeasurementValueQualityId()));
    }
