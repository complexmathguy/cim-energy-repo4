...
public class ValueToAliasAggregate {  
    ...
    public void handle(UpdateValueToAliasCommand command) throws Exception {
    	...
    	UpdateValueToAliasEvent event = new UpdateValueToAliasEvent(command.getValueToAliasId(), command.getValue(), command.getValues());        
    ...
    public void handle(AssignValuesToValueToAliasCommand command) throws Exception {
    	...
    	if ( values.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Values already contains an entity with id " + command.getAddTo().getValueToAliasId() );
    ...
    public void handle(RemoveValuesFromValueToAliasCommand command) throws Exception {
    	...
    	if ( !values.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Values does not contain an entity with id " + command.getRemoveFrom().getValueToAliasId() );

        apply(new RemoveValuesFromValueToAliasEvent(command.getValueToAliasId(), command.getRemoveFrom()));
    }
