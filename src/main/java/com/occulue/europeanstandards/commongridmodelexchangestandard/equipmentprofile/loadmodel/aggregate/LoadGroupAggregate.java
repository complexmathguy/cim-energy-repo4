...
public class LoadGroupAggregate {  
    ...
    public void handle(UpdateLoadGroupCommand command) throws Exception {
    	...
    	UpdateLoadGroupEvent event = new UpdateLoadGroupEvent(command.getLoadGroupId(), command.getLoadGroups());        
    ...
    public void handle(AssignLoadGroupsToLoadGroupCommand command) throws Exception {
    	...
    	if ( loadGroups.contains( command.getAddTo() ) )
    		throw new ProcessingException( "LoadGroups already contains an entity with id " + command.getAddTo().getLoadGroupId() );
    ...
    public void handle(RemoveLoadGroupsFromLoadGroupCommand command) throws Exception {
    	...
    	if ( !loadGroups.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "LoadGroups does not contain an entity with id " + command.getRemoveFrom().getLoadGroupId() );

        apply(new RemoveLoadGroupsFromLoadGroupEvent(command.getLoadGroupId(), command.getRemoveFrom()));
    }
