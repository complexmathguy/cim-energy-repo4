...
public class HydroPumpAggregate {  
    ...
    public void handle(UpdateHydroPumpCommand command) throws Exception {
    	...
    	UpdateHydroPumpEvent event = new UpdateHydroPumpEvent(command.getHydroPumpId(), command.getHydroPump(), command.getHydroPumps());        
    ...
    public void handle(AssignHydroPumpToHydroPumpCommand command) throws Exception {
    	...
    	if (  hydroPump != null && hydroPump.getHydroPumpId() == command.getAssignment().getHydroPumpId() )
    		throw new ProcessingException( "HydroPump already assigned with id " + command.getAssignment().getHydroPumpId() );  
    ...
    public void handle(UnAssignHydroPumpFromHydroPumpCommand command) throws Exception {
    	...
    	if (  hydroPump == null )
    		throw new ProcessingException( "HydroPump already has nothing assigned." );  
    ...
    public void handle(AssignHydroPumpsToHydroPumpCommand command) throws Exception {
    	...
    	if ( hydroPumps.contains( command.getAddTo() ) )
    		throw new ProcessingException( "HydroPumps already contains an entity with id " + command.getAddTo().getHydroPumpId() );
    ...
    public void handle(RemoveHydroPumpsFromHydroPumpCommand command) throws Exception {
    	...
    	if ( !hydroPumps.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "HydroPumps does not contain an entity with id " + command.getRemoveFrom().getHydroPumpId() );

        apply(new RemoveHydroPumpsFromHydroPumpEvent(command.getHydroPumpId(), command.getRemoveFrom()));
    }
