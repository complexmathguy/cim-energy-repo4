...
public class DCConverterUnitAggregate {  
    ...
    public void handle(UpdateDCConverterUnitCommand command) throws Exception {
    	...
    	UpdateDCConverterUnitEvent event = new UpdateDCConverterUnitEvent(command.getDCConverterUnitId(), command.getOperationMode(), command.getDCConverterUnit());        
    ...
    public void handle(AssignDCConverterUnitToDCConverterUnitCommand command) throws Exception {
    	...
    	if ( dCConverterUnit.contains( command.getAddTo() ) )
    		throw new ProcessingException( "DCConverterUnit already contains an entity with id " + command.getAddTo().getDCConverterUnitId() );

    	apply(new AssignDCConverterUnitToDCConverterUnitEvent(command.getDCConverterUnitId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveDCConverterUnitFromDCConverterUnitCommand command) throws Exception {
    	...
    	if ( !dCConverterUnit.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "DCConverterUnit does not contain an entity with id " + command.getRemoveFrom().getDCConverterUnitId() );

        apply(new RemoveDCConverterUnitFromDCConverterUnitEvent(command.getDCConverterUnitId(), command.getRemoveFrom()));
    }
