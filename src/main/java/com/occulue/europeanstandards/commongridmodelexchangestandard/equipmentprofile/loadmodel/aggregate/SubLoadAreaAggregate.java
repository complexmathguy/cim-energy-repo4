...
public class SubLoadAreaAggregate {  
    ...
    public void handle(UpdateSubLoadAreaCommand command) throws Exception {
    	...
    	UpdateSubLoadAreaEvent event = new UpdateSubLoadAreaEvent(command.getSubLoadAreaId(), command.getSubLoadAreas());        
    ...
    public void handle(AssignSubLoadAreasToSubLoadAreaCommand command) throws Exception {
    	...
    	if ( subLoadAreas.contains( command.getAddTo() ) )
    		throw new ProcessingException( "SubLoadAreas already contains an entity with id " + command.getAddTo().getSubLoadAreaId() );
    ...
    public void handle(RemoveSubLoadAreasFromSubLoadAreaCommand command) throws Exception {
    	...
    	if ( !subLoadAreas.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "SubLoadAreas does not contain an entity with id " + command.getRemoveFrom().getSubLoadAreaId() );

        apply(new RemoveSubLoadAreasFromSubLoadAreaEvent(command.getSubLoadAreaId(), command.getRemoveFrom()));
    }
