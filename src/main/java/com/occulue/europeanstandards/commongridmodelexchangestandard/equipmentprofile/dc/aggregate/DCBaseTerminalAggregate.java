...
public class DCBaseTerminalAggregate {  
    ...
    public void handle(UpdateDCBaseTerminalCommand command) throws Exception {
    	...
    	UpdateDCBaseTerminalEvent event = new UpdateDCBaseTerminalEvent(command.getDCBaseTerminalId(), command.getDCTerminals());        
    ...
    public void handle(AssignDCTerminalsToDCBaseTerminalCommand command) throws Exception {
    	...
    	if ( dCTerminals.contains( command.getAddTo() ) )
    		throw new ProcessingException( "DCTerminals already contains an entity with id " + command.getAddTo().getDCBaseTerminalId() );

    	apply(new AssignDCTerminalsToDCBaseTerminalEvent(command.getDCBaseTerminalId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveDCTerminalsFromDCBaseTerminalCommand command) throws Exception {
    	...
    	if ( !dCTerminals.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "DCTerminals does not contain an entity with id " + command.getRemoveFrom().getDCBaseTerminalId() );

        apply(new RemoveDCTerminalsFromDCBaseTerminalEvent(command.getDCBaseTerminalId(), command.getRemoveFrom()));
    }
