...
public class EquivalentInjectionAggregate {  
    ...
    public void handle(UpdateEquivalentInjectionCommand command) throws Exception {
    	...
    	UpdateEquivalentInjectionEvent event = new UpdateEquivalentInjectionEvent(command.getEquivalentInjectionId(), command.getMaxP(), command.getMaxQ(), command.getMinP(), command.getMinQ(), command.getR(), command.getR0(), command.getR2(), command.getRegulationCapability(), command.getX(), command.getX0(), command.getX2(), command.getEquivalentInjection());        
    ...
    @CommandHandler
    public void handle(AssignEquivalentInjectionToEquivalentInjectionCommand command) throws Exception {
    	...
    	if ( equivalentInjection.contains( command.getAddTo() ) )
    		throw new ProcessingException( "EquivalentInjection already contains an entity with id " + command.getAddTo().getEquivalentInjectionId() );

    	apply(new AssignEquivalentInjectionToEquivalentInjectionEvent(command.getEquivalentInjectionId(), command.getAddTo()));
    }

    @CommandHandler
    public void handle(RemoveEquivalentInjectionFromEquivalentInjectionCommand command) throws Exception {
    	...
    	if ( !equivalentInjection.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "EquivalentInjection does not contain an entity with id " + command.getRemoveFrom().getEquivalentInjectionId() );

        apply(new RemoveEquivalentInjectionFromEquivalentInjectionEvent(command.getEquivalentInjectionId(), command.getRemoveFrom()));
    }
