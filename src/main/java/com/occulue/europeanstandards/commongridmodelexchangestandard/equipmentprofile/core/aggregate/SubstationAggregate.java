...
public class SubstationAggregate {  
    ...
    public void handle(UpdateSubstationCommand command) throws Exception {
    	...
    	UpdateSubstationEvent event = new UpdateSubstationEvent(command.getSubstationId(), command.getSubstations());        
    ...
    public void handle(AssignSubstationsToSubstationCommand command) throws Exception {
    	...
    	if ( substations.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Substations already contains an entity with id " + command.getAddTo().getSubstationId() );
    ...
    public void handle(RemoveSubstationsFromSubstationCommand command) throws Exception {
    	...
    	if ( !substations.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Substations does not contain an entity with id " + command.getRemoveFrom().getSubstationId() );

        apply(new RemoveSubstationsFromSubstationEvent(command.getSubstationId(), command.getRemoveFrom()));
    }
