...
public class ACDCConverterDCTerminalAggregate {  
    ...
    public void handle(UpdateACDCConverterDCTerminalCommand command) throws Exception {
    	...
    	UpdateACDCConverterDCTerminalEvent event = new UpdateACDCConverterDCTerminalEvent(command.getACDCConverterDCTerminalId(), command.getPolarity(), command.getDCTerminals());        
    ...
    public void handle(AssignDCTerminalsToACDCConverterDCTerminalCommand command) throws Exception {
    	...
    	if ( dCTerminals.contains( command.getAddTo() ) )
    		throw new ProcessingException( "DCTerminals already contains an entity with id " + command.getAddTo().getACDCConverterDCTerminalId() );

    	apply(new AssignDCTerminalsToACDCConverterDCTerminalEvent(command.getACDCConverterDCTerminalId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveDCTerminalsFromACDCConverterDCTerminalCommand command) throws Exception {
    	...
    	if ( !dCTerminals.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "DCTerminals does not contain an entity with id " + command.getRemoveFrom().getACDCConverterDCTerminalId() );

        apply(new RemoveDCTerminalsFromACDCConverterDCTerminalEvent(command.getACDCConverterDCTerminalId(), command.getRemoveFrom()));
    }
