...
public class CurveDataAggregate {  
    ...
    public void handle(UpdateCurveDataCommand command) throws Exception {
    	...
    	UpdateCurveDataEvent event = new UpdateCurveDataEvent(command.getCurveDataId(), command.getXvalue(), command.getY1value(), command.getY2value(), command.getCurveDatas());        
    ...
    public void handle(AssignCurveDatasToCurveDataCommand command) throws Exception {
    	...
    	if ( curveDatas.contains( command.getAddTo() ) )
    		throw new ProcessingException( "CurveDatas already contains an entity with id " + command.getAddTo().getCurveDataId() );
    ...
    public void handle(RemoveCurveDatasFromCurveDataCommand command) throws Exception {
    	...
    	if ( !curveDatas.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "CurveDatas does not contain an entity with id " + command.getRemoveFrom().getCurveDataId() );

        apply(new RemoveCurveDatasFromCurveDataEvent(command.getCurveDataId(), command.getRemoveFrom()));
    }
