...
public class HydroGeneratingUnitAggregate {  
    ...
    public void handle(UpdateHydroGeneratingUnitCommand command) throws Exception {
    	...
    	UpdateHydroGeneratingUnitEvent event = new UpdateHydroGeneratingUnitEvent(command.getHydroGeneratingUnitId(), command.getEnergyConversionCapability(), command.getHydroGeneratingUnits());        
    ...
    @CommandHandler
    public void handle(AssignHydroGeneratingUnitsToHydroGeneratingUnitCommand command) throws Exception {
    	...
    	if ( hydroGeneratingUnits.contains( command.getAddTo() ) )
    		throw new ProcessingException( "HydroGeneratingUnits already contains an entity with id " + command.getAddTo().getHydroGeneratingUnitId() );

    	apply(new AssignHydroGeneratingUnitsToHydroGeneratingUnitEvent(command.getHydroGeneratingUnitId(), command.getAddTo()));
    }

    @CommandHandler
    public void handle(RemoveHydroGeneratingUnitsFromHydroGeneratingUnitCommand command) throws Exception {
    	...
    	if ( !hydroGeneratingUnits.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "HydroGeneratingUnits does not contain an entity with id " + command.getRemoveFrom().getHydroGeneratingUnitId() );

        apply(new RemoveHydroGeneratingUnitsFromHydroGeneratingUnitEvent(command.getHydroGeneratingUnitId(), command.getRemoveFrom()));
    }
