...
public class FossilFuelAggregate {  
    ...
    public void handle(UpdateFossilFuelCommand command) throws Exception {
    	...
    	UpdateFossilFuelEvent event = new UpdateFossilFuelEvent(command.getFossilFuelId(), command.getFossilFuelType(), command.getFossilFuels());        
    ...
    public void handle(AssignFossilFuelsToFossilFuelCommand command) throws Exception {
    	...
    	if ( fossilFuels.contains( command.getAddTo() ) )
    		throw new ProcessingException( "FossilFuels already contains an entity with id " + command.getAddTo().getFossilFuelId() );
    ...
    public void handle(RemoveFossilFuelsFromFossilFuelCommand command) throws Exception {
    	...
    	if ( !fossilFuels.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "FossilFuels does not contain an entity with id " + command.getRemoveFrom().getFossilFuelId() );

        apply(new RemoveFossilFuelsFromFossilFuelEvent(command.getFossilFuelId(), command.getRemoveFrom()));
    }
