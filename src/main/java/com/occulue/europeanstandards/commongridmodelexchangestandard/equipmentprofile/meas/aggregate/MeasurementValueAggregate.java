...
public class MeasurementValueAggregate {  
    ...
    public void handle(UpdateMeasurementValueCommand command) throws Exception {
    	...
    	UpdateMeasurementValueEvent event = new UpdateMeasurementValueEvent(command.getMeasurementValueId(), command.getSensorAccuracy(), command.getTimeStamp(), command.getMeasurementValues());        
    ...
    public void handle(AssignMeasurementValuesToMeasurementValueCommand command) throws Exception {
    	...
    	if ( measurementValues.contains( command.getAddTo() ) )
    		throw new ProcessingException( "MeasurementValues already contains an entity with id " + command.getAddTo().getMeasurementValueId() );

    	apply(new AssignMeasurementValuesToMeasurementValueEvent(command.getMeasurementValueId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveMeasurementValuesFromMeasurementValueCommand command) throws Exception {
    	...
    	if ( !measurementValues.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "MeasurementValues does not contain an entity with id " + command.getRemoveFrom().getMeasurementValueId() );

        apply(new RemoveMeasurementValuesFromMeasurementValueEvent(command.getMeasurementValueId(), command.getRemoveFrom()));
    }
