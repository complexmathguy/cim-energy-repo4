...
public class AccumulatorLimitAggregate {  
    ...
    public void handle(UpdateAccumulatorLimitCommand command) throws Exception {
    	...
    	UpdateAccumulatorLimitEvent event = new UpdateAccumulatorLimitEvent(command.getAccumulatorLimitId(), command.getValue(), command.getLimits());        
    ...
    public void handle(AssignLimitsToAccumulatorLimitCommand command) throws Exception {
    	...
    	if ( limits.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Limits already contains an entity with id " + command.getAddTo().getAccumulatorLimitId() );

    	apply(new AssignLimitsToAccumulatorLimitEvent(command.getAccumulatorLimitId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveLimitsFromAccumulatorLimitCommand command) throws Exception {
    	...
    	if ( !limits.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Limits does not contain an entity with id " + command.getRemoveFrom().getAccumulatorLimitId() );

        apply(new RemoveLimitsFromAccumulatorLimitEvent(command.getAccumulatorLimitId(), command.getRemoveFrom()));
    }
