...
public class DiscreteValueAggregate {  
    ...
    public void handle(UpdateDiscreteValueCommand command) throws Exception {
    	...
    	UpdateDiscreteValueEvent event = new UpdateDiscreteValueEvent(command.getDiscreteValueId(), command.getValue(), command.getDiscreteValues());        
    ...
    public void handle(AssignDiscreteValuesToDiscreteValueCommand command) throws Exception {
    	...
    	if ( discreteValues.contains( command.getAddTo() ) )
    		throw new ProcessingException( "DiscreteValues already contains an entity with id " + command.getAddTo().getDiscreteValueId() );

    	apply(new AssignDiscreteValuesToDiscreteValueEvent(command.getDiscreteValueId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveDiscreteValuesFromDiscreteValueCommand command) throws Exception {
    	...
    	if ( !discreteValues.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "DiscreteValues does not contain an entity with id " + command.getRemoveFrom().getDiscreteValueId() );

        apply(new RemoveDiscreteValuesFromDiscreteValueEvent(command.getDiscreteValueId(), command.getRemoveFrom()));
    }
