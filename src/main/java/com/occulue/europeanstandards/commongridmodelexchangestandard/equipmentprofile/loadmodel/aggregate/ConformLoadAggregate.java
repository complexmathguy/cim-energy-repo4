...
public class ConformLoadAggregate {  
    ...
    public void handle(UpdateConformLoadCommand command) throws Exception {
    	...
    	UpdateConformLoadEvent event = new UpdateConformLoadEvent(command.getConformLoadId(), command.getEnergyConsumers());        
    ...
    public void handle(AssignEnergyConsumersToConformLoadCommand command) throws Exception {
    	...
    	if ( energyConsumers.contains( command.getAddTo() ) )
    		throw new ProcessingException( "EnergyConsumers already contains an entity with id " + command.getAddTo().getConformLoadId() );

    	apply(new AssignEnergyConsumersToConformLoadEvent(command.getConformLoadId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveEnergyConsumersFromConformLoadCommand command) throws Exception {
    	...
    	if ( !energyConsumers.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "EnergyConsumers does not contain an entity with id " + command.getRemoveFrom().getConformLoadId() );

        apply(new RemoveEnergyConsumersFromConformLoadEvent(command.getConformLoadId(), command.getRemoveFrom()));
    }
