...
public class AnalogLimitAggregate {  
    ...
    public void handle(UpdateAnalogLimitCommand command) throws Exception {
    	...
    	UpdateAnalogLimitEvent event = new UpdateAnalogLimitEvent(command.getAnalogLimitId(), command.getValue(), command.getLimits());        
    ...
    public void handle(AssignLimitsToAnalogLimitCommand command) throws Exception {
    	...
    	if ( limits.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Limits already contains an entity with id " + command.getAddTo().getAnalogLimitId() );
    ...
    public void handle(RemoveLimitsFromAnalogLimitCommand command) throws Exception {
    	...
    	if ( !limits.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Limits does not contain an entity with id " + command.getRemoveFrom().getAnalogLimitId() );

        apply(new RemoveLimitsFromAnalogLimitEvent(command.getAnalogLimitId(), command.getRemoveFrom()));
    }
