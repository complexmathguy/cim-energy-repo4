...
public class NonConformLoadAggregate {  
    ...
    public void handle(UpdateNonConformLoadCommand command) throws Exception {
    	...
    	UpdateNonConformLoadEvent event = new UpdateNonConformLoadEvent(command.getNonConformLoadId(), command.getEnergyConsumers());        
    ...
    public void handle(AssignEnergyConsumersToNonConformLoadCommand command) throws Exception {
    	...
    	if ( energyConsumers.contains( command.getAddTo() ) )
    		throw new ProcessingException( "EnergyConsumers already contains an entity with id " + command.getAddTo().getNonConformLoadId() );

    	apply(new AssignEnergyConsumersToNonConformLoadEvent(command.getNonConformLoadId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveEnergyConsumersFromNonConformLoadCommand command) throws Exception {
    	...
    	if ( !energyConsumers.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "EnergyConsumers does not contain an entity with id " + command.getRemoveFrom().getNonConformLoadId() );

        apply(new RemoveEnergyConsumersFromNonConformLoadEvent(command.getNonConformLoadId(), command.getRemoveFrom()));
    }
