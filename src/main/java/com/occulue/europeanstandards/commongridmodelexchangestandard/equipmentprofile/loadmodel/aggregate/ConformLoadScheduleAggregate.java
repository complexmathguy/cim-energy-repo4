...
public class ConformLoadScheduleAggregate {  
    ...
    public void handle(UpdateConformLoadScheduleCommand command) throws Exception {
    	...
    	UpdateConformLoadScheduleEvent event = new UpdateConformLoadScheduleEvent(command.getConformLoadScheduleId(), command.getConformLoadSchedules());        
    ...
    @CommandHandler
    public void handle(AssignConformLoadSchedulesToConformLoadScheduleCommand command) throws Exception {
    	...
    	if ( conformLoadSchedules.contains( command.getAddTo() ) )
    		throw new ProcessingException( "ConformLoadSchedules already contains an entity with id " + command.getAddTo().getConformLoadScheduleId() );

    	apply(new AssignConformLoadSchedulesToConformLoadScheduleEvent(command.getConformLoadScheduleId(), command.getAddTo()));
    }

    @CommandHandler
    public void handle(RemoveConformLoadSchedulesFromConformLoadScheduleCommand command) throws Exception {
    	...
    	if ( !conformLoadSchedules.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "ConformLoadSchedules does not contain an entity with id " + command.getRemoveFrom().getConformLoadScheduleId() );

        apply(new RemoveConformLoadSchedulesFromConformLoadScheduleEvent(command.getConformLoadScheduleId(), command.getRemoveFrom()));
    }
