...
public class TapScheduleAggregate {  
    ...
    public void handle(UpdateTapScheduleCommand command) throws Exception {
    	...
    	UpdateTapScheduleEvent event = new UpdateTapScheduleEvent(command.getTapScheduleId(), command.getTapSchedules());        
    ...
    public void handle(AssignTapSchedulesToTapScheduleCommand command) throws Exception {
    	...
    	if ( tapSchedules.contains( command.getAddTo() ) )
    		throw new ProcessingException( "TapSchedules already contains an entity with id " + command.getAddTo().getTapScheduleId() );
    ...
    public void handle(RemoveTapSchedulesFromTapScheduleCommand command) throws Exception {
    	...
    	if ( !tapSchedules.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "TapSchedules does not contain an entity with id " + command.getRemoveFrom().getTapScheduleId() );

        apply(new RemoveTapSchedulesFromTapScheduleEvent(command.getTapScheduleId(), command.getRemoveFrom()));
    }
