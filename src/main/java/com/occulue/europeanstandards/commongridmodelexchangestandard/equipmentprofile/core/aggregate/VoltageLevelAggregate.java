...
public class VoltageLevelAggregate {  
    ...
    public void handle(UpdateVoltageLevelCommand command) throws Exception {
    	...
    	UpdateVoltageLevelEvent event = new UpdateVoltageLevelEvent(command.getVoltageLevelId(), command.getHighVoltageLimit(), command.getLowVoltageLimit(), command.getVoltageLevel(), command.getVoltageLevels());        
    ...
    public void handle(AssignVoltageLevelToVoltageLevelCommand command) throws Exception {
    	...
    	if ( voltageLevel.contains( command.getAddTo() ) )
    		throw new ProcessingException( "VoltageLevel already contains an entity with id " + command.getAddTo().getVoltageLevelId() );

    	apply(new AssignVoltageLevelToVoltageLevelEvent(command.getVoltageLevelId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveVoltageLevelFromVoltageLevelCommand command) throws Exception {
    	...
    	if ( !voltageLevel.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "VoltageLevel does not contain an entity with id " + command.getRemoveFrom().getVoltageLevelId() );

        apply(new RemoveVoltageLevelFromVoltageLevelEvent(command.getVoltageLevelId(), command.getRemoveFrom()));
    }
    @CommandHandler
    public void handle(AssignVoltageLevelsToVoltageLevelCommand command) throws Exception {
    	...
    	if ( voltageLevels.contains( command.getAddTo() ) )
    		throw new ProcessingException( "VoltageLevels already contains an entity with id " + command.getAddTo().getVoltageLevelId() );

    	apply(new AssignVoltageLevelsToVoltageLevelEvent(command.getVoltageLevelId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveVoltageLevelsFromVoltageLevelCommand command) throws Exception {
    	...
    	if ( !voltageLevels.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "VoltageLevels does not contain an entity with id " + command.getRemoveFrom().getVoltageLevelId() );

        apply(new RemoveVoltageLevelsFromVoltageLevelEvent(command.getVoltageLevelId(), command.getRemoveFrom()));
    }
