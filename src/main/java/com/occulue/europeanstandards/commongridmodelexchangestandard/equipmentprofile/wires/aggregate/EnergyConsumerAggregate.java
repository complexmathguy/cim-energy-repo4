...
public class EnergyConsumerAggregate {  
    ...
    public void handle(UpdateEnergyConsumerCommand command) throws Exception {
    	...
    	UpdateEnergyConsumerEvent event = new UpdateEnergyConsumerEvent(command.getEnergyConsumerId(), command.getPfixed(), command.getPfixedPct(), command.getQfixed(), command.getQfixedPct(), command.getEnergyConsumer());        
    ...
    public void handle(AssignEnergyConsumerToEnergyConsumerCommand command) throws Exception {
    	...
    	if ( energyConsumer.contains( command.getAddTo() ) )
    		throw new ProcessingException( "EnergyConsumer already contains an entity with id " + command.getAddTo().getEnergyConsumerId() );

    	apply(new AssignEnergyConsumerToEnergyConsumerEvent(command.getEnergyConsumerId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveEnergyConsumerFromEnergyConsumerCommand command) throws Exception {
    	...
    	if ( !energyConsumer.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "EnergyConsumer does not contain an entity with id " + command.getRemoveFrom().getEnergyConsumerId() );

        apply(new RemoveEnergyConsumerFromEnergyConsumerEvent(command.getEnergyConsumerId(), command.getRemoveFrom()));
    }
