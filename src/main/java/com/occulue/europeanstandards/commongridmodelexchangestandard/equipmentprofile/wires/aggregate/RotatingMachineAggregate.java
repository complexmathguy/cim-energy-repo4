...
public class RotatingMachineAggregate {  
    ...
    public void handle(UpdateRotatingMachineCommand command) throws Exception {
    	...
    	UpdateRotatingMachineEvent event = new UpdateRotatingMachineEvent(command.getRotatingMachineId(), command.getRatedPowerFactor(), command.getRatedS(), command.getRatedU(), command.getRotatingMachine());        
    ...
    public void handle(AssignRotatingMachineToRotatingMachineCommand command) throws Exception {
    	...
    	if ( rotatingMachine.contains( command.getAddTo() ) )
    		throw new ProcessingException( "RotatingMachine already contains an entity with id " + command.getAddTo().getRotatingMachineId() );

    	apply(new AssignRotatingMachineToRotatingMachineEvent(command.getRotatingMachineId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveRotatingMachineFromRotatingMachineCommand command) throws Exception {
    	...
    	if ( !rotatingMachine.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "RotatingMachine does not contain an entity with id " + command.getRemoveFrom().getRotatingMachineId() );

        apply(new RemoveRotatingMachineFromRotatingMachineEvent(command.getRotatingMachineId(), command.getRemoveFrom()));
    }
