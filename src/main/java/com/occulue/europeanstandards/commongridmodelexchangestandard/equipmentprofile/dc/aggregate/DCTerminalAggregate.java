...
public class DCTerminalAggregate {  
    ...
    public void handle(UpdateDCTerminalCommand command) throws Exception {
    	...
    	UpdateDCTerminalEvent event = new UpdateDCTerminalEvent(command.getDCTerminalId(), command.getDCTerminals());        
    ...
    public void handle(AssignDCTerminalsToDCTerminalCommand command) throws Exception {
    	...
    	if ( dCTerminals.contains( command.getAddTo() ) )
    		throw new ProcessingException( "DCTerminals already contains an entity with id " + command.getAddTo().getDCTerminalId() );
    ...
    public void handle(RemoveDCTerminalsFromDCTerminalCommand command) throws Exception {
    	...
    	if ( !dCTerminals.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "DCTerminals does not contain an entity with id " + command.getRemoveFrom().getDCTerminalId() );

        apply(new RemoveDCTerminalsFromDCTerminalEvent(command.getDCTerminalId(), command.getRemoveFrom()));
    }
