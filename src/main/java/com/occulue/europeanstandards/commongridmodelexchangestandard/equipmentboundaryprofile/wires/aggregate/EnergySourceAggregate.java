...
public class EnergySourceAggregate {  
    ...
    public void handle(UpdateEnergySourceCommand command) throws Exception {
    	...
    	UpdateEnergySourceEvent event = new UpdateEnergySourceEvent(command.getEnergySourceId(), command.getEnergySource());        
    ...
    public void handle(AssignEnergySourceToEnergySourceCommand command) throws Exception {
    	...
    	if ( energySource.contains( command.getAddTo() ) )
    		throw new ProcessingException( "EnergySource already contains an entity with id " + command.getAddTo().getEnergySourceId() );

    	apply(new AssignEnergySourceToEnergySourceEvent(command.getEnergySourceId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveEnergySourceFromEnergySourceCommand command) throws Exception {
    	...
    	if ( !energySource.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "EnergySource does not contain an entity with id " + command.getRemoveFrom().getEnergySourceId() );

        apply(new RemoveEnergySourceFromEnergySourceEvent(command.getEnergySourceId(), command.getRemoveFrom()));
    }
