...
public class EquipmentAggregate {  
    ...
    public void handle(UpdateEquipmentCommand command) throws Exception {
    	...
    	UpdateEquipmentEvent event = new UpdateEquipmentEvent(command.getEquipmentId(), command.getEquipments());        
    ...
    public void handle(AssignEquipmentsToEquipmentCommand command) throws Exception {
    	...
    	if ( equipments.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Equipments already contains an entity with id " + command.getAddTo().getEquipmentId() );
    ...
    public void handle(RemoveEquipmentsFromEquipmentCommand command) throws Exception {
    	...
    	if ( !equipments.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Equipments does not contain an entity with id " + command.getRemoveFrom().getEquipmentId() );

        apply(new RemoveEquipmentsFromEquipmentEvent(command.getEquipmentId(), command.getRemoveFrom()));
    }
