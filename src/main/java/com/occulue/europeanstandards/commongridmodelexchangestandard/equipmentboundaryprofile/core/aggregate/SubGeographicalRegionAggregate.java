...
public class SubGeographicalRegionAggregate {  
    ...
    public void handle(UpdateSubGeographicalRegionCommand command) throws Exception {
    	...
    	UpdateSubGeographicalRegionEvent event = new UpdateSubGeographicalRegionEvent(command.getSubGeographicalRegionId(), command.getRegions());        
    ...
    public void handle(AssignRegionsToSubGeographicalRegionCommand command) throws Exception {
    	...
    	if ( regions.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Regions already contains an entity with id " + command.getAddTo().getSubGeographicalRegionId() );

    	apply(new AssignRegionsToSubGeographicalRegionEvent(command.getSubGeographicalRegionId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveRegionsFromSubGeographicalRegionCommand command) throws Exception {
    	...
    	if ( !regions.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Regions does not contain an entity with id " + command.getRemoveFrom().getSubGeographicalRegionId() );

        apply(new RemoveRegionsFromSubGeographicalRegionEvent(command.getSubGeographicalRegionId(), command.getRemoveFrom()));
    }
