...
public class RemoteInputSignalAggregate {  
    ...
    public void handle(UpdateRemoteInputSignalCommand command) throws Exception {
    	...
    	UpdateRemoteInputSignalEvent event = new UpdateRemoteInputSignalEvent(command.getRemoteInputSignalId(), command.getRemoteSignalType(), command.getRemoteInputSignal());        
    ...
    public void handle(AssignRemoteInputSignalToRemoteInputSignalCommand command) throws Exception {
    	...
    	if ( remoteInputSignal.contains( command.getAddTo() ) )
    		throw new ProcessingException( "RemoteInputSignal already contains an entity with id " + command.getAddTo().getRemoteInputSignalId() );

    	apply(new AssignRemoteInputSignalToRemoteInputSignalEvent(command.getRemoteInputSignalId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveRemoteInputSignalFromRemoteInputSignalCommand command) throws Exception {
    	...
    	if ( !remoteInputSignal.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "RemoteInputSignal does not contain an entity with id " + command.getRemoveFrom().getRemoteInputSignalId() );

        apply(new RemoveRemoteInputSignalFromRemoteInputSignalEvent(command.getRemoteInputSignalId(), command.getRemoveFrom()));
    }
