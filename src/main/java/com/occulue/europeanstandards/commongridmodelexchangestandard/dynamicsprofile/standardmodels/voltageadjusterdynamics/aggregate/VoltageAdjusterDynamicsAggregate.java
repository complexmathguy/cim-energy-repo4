...
public class VoltageAdjusterDynamicsAggregate {  
    ...
    public void handle(UpdateVoltageAdjusterDynamicsCommand command) throws Exception {
    	...
    	UpdateVoltageAdjusterDynamicsEvent event = new UpdateVoltageAdjusterDynamicsEvent(command.getVoltageAdjusterDynamicsId(), command.getVoltageAdjusterDynamics());        
    ...
    @CommandHandler
    public void handle(AssignVoltageAdjusterDynamicsToVoltageAdjusterDynamicsCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignVoltageAdjusterDynamicsToVoltageAdjusterDynamicsCommand" );
    	
    	if (  voltageAdjusterDynamics != null && voltageAdjusterDynamics.getVoltageAdjusterDynamicsId() == command.getAssignment().getVoltageAdjusterDynamicsId() )
    		throw new ProcessingException( "VoltageAdjusterDynamics already assigned with id " + command.getAssignment().getVoltageAdjusterDynamicsId() );  
    		
        apply(new AssignVoltageAdjusterDynamicsToVoltageAdjusterDynamicsEvent(command.getVoltageAdjusterDynamicsId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignVoltageAdjusterDynamicsFromVoltageAdjusterDynamicsCommand command) throws Exception {
    	LOGGER.info( "Handlign command UnAssignVoltageAdjusterDynamicsFromVoltageAdjusterDynamicsCommand" );

    	if (  voltageAdjusterDynamics == null )
    		throw new ProcessingException( "VoltageAdjusterDynamics already has nothing assigned." );  

    	apply(new UnAssignVoltageAdjusterDynamicsFromVoltageAdjusterDynamicsEvent(command.getVoltageAdjusterDynamicsId()));
    }
