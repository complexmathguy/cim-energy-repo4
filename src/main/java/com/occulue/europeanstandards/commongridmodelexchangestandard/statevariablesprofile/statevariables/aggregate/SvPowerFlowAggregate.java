...
public class SvPowerFlowAggregate {  
    ...
    public void handle(UpdateSvPowerFlowCommand command) throws Exception {
    	...
    	UpdateSvPowerFlowEvent event = new UpdateSvPowerFlowEvent(command.getSvPowerFlowId(), command.getP(), command.getQ(), command.getSvPowerFlow());        
    ...
    public void handle(AssignSvPowerFlowToSvPowerFlowCommand command) throws Exception {
    	...
    	if (  svPowerFlow != null && svPowerFlow.getSvPowerFlowId() == command.getAssignment().getSvPowerFlowId() )
    		throw new ProcessingException( "SvPowerFlow already assigned with id " + command.getAssignment().getSvPowerFlowId() );  
    		
        apply(new AssignSvPowerFlowToSvPowerFlowEvent(command.getSvPowerFlowId(), command.getAssignment()));
    }
