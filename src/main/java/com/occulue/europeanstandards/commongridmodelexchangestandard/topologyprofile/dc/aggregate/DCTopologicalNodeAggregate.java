...
public class DCTopologicalNodeAggregate {  
    ...
    public void handle(UpdateDCTopologicalNodeCommand command) throws Exception {
    	...
    	UpdateDCTopologicalNodeEvent event = new UpdateDCTopologicalNodeEvent(command.getDCTopologicalNodeId(), command.getDCTopologicalNode());        
    ...
    public void handle(AssignDCTopologicalNodeToDCTopologicalNodeCommand command) throws Exception {
    	...
    	if ( dCTopologicalNode.contains( command.getAddTo() ) )
    		throw new ProcessingException( "DCTopologicalNode already contains an entity with id " + command.getAddTo().getDCTopologicalNodeId() );

    	apply(new AssignDCTopologicalNodeToDCTopologicalNodeEvent(command.getDCTopologicalNodeId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveDCTopologicalNodeFromDCTopologicalNodeCommand command) throws Exception {
    	...
    	if ( !dCTopologicalNode.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "DCTopologicalNode does not contain an entity with id " + command.getRemoveFrom().getDCTopologicalNodeId() );

        apply(new RemoveDCTopologicalNodeFromDCTopologicalNodeEvent(command.getDCTopologicalNodeId(), command.getRemoveFrom()));
    }
